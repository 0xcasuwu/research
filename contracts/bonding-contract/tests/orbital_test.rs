...
// Helper function to get the bond orbital ID for a bond
// This is a workaround since get_bond_orbital_id is private
fn get_bond_orbital_id(contract: &BondingContractAlkane, bond_id: u128) -> AlkaneId {
    // In our test environment, we know that orbital IDs are created with a specific pattern
    AlkaneId {
        block: 2,
        tx: 1000 + bond_id,
    }
...
fn test_orbital_bond_lifecycle() {
    ...
    // Get the orbital ID using our helper function
    let orbital_id = get_bond_orbital_id(&contract, 0);
    ...
    // Set up the mock context for redeeming with the orbital token
    let context = Context {
        caller: caller.clone(),
        myself: myself.clone(),
        incoming_alkanes: AlkaneTransfers(vec![
...
fn test_orbital_multiple_bonds() {
    ...
    // Get the first orbital ID using our helper function
    let orbital_id_1 = get_bond_orbital_id(&contract, 0);
    ...
    // Get the second orbital ID using our helper function
    let orbital_id_2 = get_bond_orbital_id(&contract, 1);
