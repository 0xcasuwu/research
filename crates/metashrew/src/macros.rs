...
///
/// ```
/// use metashrew::metashrew_indexer;
                        ...
                        match $crate::view::parse_view_input::<$input_type>() {
                            Ok(input) => {
                                match indexer.indexer.$view_method(input) {
                                    Ok(output) => {
                                        // Serialize the output to JSON bytes
            ...
            pub extern "C" fn $view_method() -> i32 {
                unsafe {
                    ...
                    if let Some(indexer) = &INDEXER_INSTANCE {
                        ...
                        // Parse the request using serde_json
                        let $request_param = match serde_json::from_slice::<$request_type>(&input_bytes) {
                            Ok(req) => req,
                            Err(e) => {
                        ...
                        match (|$request_param: $request_type| -> anyhow::Result<$response_type> {
                            ...
                            Ok(response) => {
                                // Serialize the response using serde_json
...
///
/// ```
/// use metashrew::native_binary;
/// use metashrew::indexer::{Indexer, KeyValueStore, NativeIndexer};
/// use anyhow::Result;
...
///     fn view_functions(&self) -> HashMap<String, Box<dyn metashrew::indexer::ViewFunctionWrapper>> {
